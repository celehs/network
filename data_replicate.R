...
data.res1 <- vector("list", length(interest.list))
names(data.res1) <- interest.list
data.res2 <- vector("list", length(interest.list))
names(data.res2) <- interest.list
data.res3 <- vector("list", length(interest.list))
names(data.res3) <- interest.list
for (ii in 1:length(interest.list)){
  ...
  data.res1[[interest]] <- res1
  data.res2[[interest]] <- res2
  data.res3[[interest]] <- res3

}

saveRDS(data.res1, 'data_res1.rds')
saveRDS(data.res2, 'data_res2.rds')
saveRDS(data.res3, 'data_res3.rds')

net <- vector("list", length(interest.list))
names(net) <- paste0("network_[",gsub(":",".",interest.list),"]")
for (ii in 1:length(interest.list)){
  interest=interest.list[ii]
  interest.label=dict.combine[dict.combine$Variable==interest,"Description"]
  tryCatch(net.fun(interest, interest.label, data.res1[[ii]], data.res2[[ii]], data.res3[[ii]]), error=function(e) NA)
  #saveWidget(net, file = paste0("network_[",gsub(":",".",interest),"].html"))
  net[[paste0("network_[",gsub(":",".",interest),"]")]] = net
}
